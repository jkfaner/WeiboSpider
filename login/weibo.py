...
class weiboPC():
    ...
    def login(self, username, password, crack_captcha_func=None, **kwargs):
        ...
        # 安全验证
            ...
            response = self.session.get(response_json['blog']['errurl'])
            ...
            if response_json['retcode'] not in [100000]:
                raise RuntimeError(response_json.get('msg'))
            login_url = response_json['blog']['url']
...
class weiboScanqr(ExtractorApi):
    '''扫码登录微博'''
    is_callable = True
    ...
    def login(self, username='', password='', crack_captcha_func=None, **kwargs):
        ...
        qrid = self.find_first_data(response_json,"qrid")
        imageurl = 'https:' + self.find_first_data(response_json,"image")
        ...
        params = {
            ...
            'cdult': '3',
            'domain': 'weibo.com',
            'alt': self.find_first_data(response_json,"alt"),
            'savestate': '30',
        ...
        infos_return = {'username': username,"uid":response_json.get('uid')}
        infos_return.update(response_json)
        return infos_return, self.session
